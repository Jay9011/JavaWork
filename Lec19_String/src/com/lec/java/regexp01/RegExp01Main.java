...
public class RegExp01Main {
	...
	public static void main(String[] args) {
		...
		String input;
		String regex;	// 정규표현식
		Pattern pat;	// 정규표현식 구현
		Matcher matcher;// 정규표현식 매칭
		...
		regex = "My....";	// My 로 시작하고 임의의 문자 4개가 따라오는 패턴
		pat = Pattern.compile(regex);	// Patten 객체 생성
		...
		// 위의 코드를 다시 실행한다면? , 다음 패턴 매칭을 시도한다. --> 결과 실패!
		if(matcher.find()) {
			System.out.println("find() 성공");
			System.out.println(matcher.group() + "{" + matcher.start() + "~" + matcher.end() + "}");
		}else {
