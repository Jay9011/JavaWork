...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception{
		...
		int year = now.getYear();	// 연도는 1900년 이후 경과 년도
		int month = now.getMonth();	// 0부터 시작
		int day = now.getDate();	// 일
		int weekDay = now.getDay();	// 요일	: 일요일이 0
		int hour = now.getHours();
		int minute = now.getMinutes();
		int second = now.getSeconds();
		System.out.println((year + 1900) + "년 " + (month + 1) + "월 " + day + "일 " + hour + "시 " + minute + "분 " + second+ "초");
		...
		long baseTime = System.currentTimeMillis();		// 현재 시간을 ms 단위로 리턴
...
//		System.out.println(baseTime);
		...
		Date date = 
				new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.S", Locale.KOREA).parse(oldstring);
		...
		System.out.println("경과시간 : " + lapTime);
