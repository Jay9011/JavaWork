...
public class Collection14Main {
	...
	public static void main(String[] args) throws ParseException {
		...
		// "2020-08-16" --> Date 객체
		list2.add(new SimpleDateFormat("yyyy-MM-dd").parse("2020-08-16"));
		...
		Collections.sort(list3);	// Student 에 Comparable<> 구현 안되어 있으면 에러!
		// Comparable<> 구현 후 실행
		System.out.println(list3);
		System.out.println();
		...
		Collections.sort(list3, new Asc());	// 밑에서 만든 '이름' 오름차순 정렬
		System.out.println(list3);
		
		Collections.sort(list3, new Desc()); // 밑에서 만든 '이름' 내림차순 정렬
		System.out.println(list3);
...
//		return 0;							// 같으면 0
		...
		// 점수 내림차순
		if(o.point > this.point) return 1;	// 내가 더 작으면 양수
		if(this.point > o.point) return -1;	// 내가 더 크면 음수
		return 0;							// 같으면 0
...
// Student 를 이름(name) 으로 정렬해줄 수 있는 Comparator<> 만들기
